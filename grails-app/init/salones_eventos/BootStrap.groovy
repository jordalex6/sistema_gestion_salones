...
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        /* El resto del codigo */
        def salon3 = new Salon(nombre:'Zafiro', propietario:clientUser.getPropietario(), direccion:'La Carrera', telefono:'3834333333', capacidad:300, precio:1000.0, tipoSalon:'normal')
        if(!salon3.save(flush:true)){
            salon3.errors.allErrors.each{println it}
        }else{
            println("Salon agregado a BD")
        }
        def salon4 = new Salon(nombre:'Ruby', propietario:clientUser.getPropietario(), direccion:'Piedra Blanca', telefono:'3834444444', capacidad:200, precio:55000.0, tipoSalon:'normal')
        if(!salon4.save(flush:true)){
            salon4.errors.allErrors.each{println it}
        }else{
            println("Salon agregado a BD")
        }
        def salon5 = new Salon(nombre:'Diamante', propietario:propUser.getPropietario(), direccion:'San Isidro', telefono:'3834555555', capacidad:1000, precio:35500.0, tipoSalon:'normal')
        if(!salon5.save(flush:true)){
            salon5.errors.allErrors.each{println it}
        }else{
            println("Salon agregado a BD")
        }
        def salon6 = new Salon(nombre:'Carbon', propietario:propUser.getPropietario(), direccion:'Capital', telefono:'3834666666', capacidad:450, precio:42000.0, tipoSalon:'normal')
        if(!salon6.save(flush:true)){
            salon6.errors.allErrors.each{println it}
        }else{
            println("Salon agregado a BD")
        }
        def salon7 = new Salon(nombre:'Perla', propietario:propUser.getPropietario(), direccion:'Capital', telefono:'3834777777', capacidad:3000, precio:2358.0, tipoSalon:'normal')
